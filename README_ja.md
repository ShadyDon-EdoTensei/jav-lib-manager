...
- 👩 **女優アバター** - 945+の女優プロフィール（アバター、カップサイズ、身長、生年）を閲覧
...
- 女優名をクリックしてアバターとプロフィール（カップ、身長、生年）を表示
- タグカテゴリー
- ファイル情報（パス、サイズ、日付）

#### 女優アバター閲覧

1. ツールバーの **「女優头像」** ボタンをクリックして女優ブラウザを開く
2. 945+の女優カードを閲覧、名前検索や「アバターあり」フィルタが可能
3. **「全アバターをダウンロード」** で欠落しているアバターを一括ダウンロード
4. 動画詳細パネルで女優名をクリックするとアバターと情報が表示されます
5. 女優カードをダブルクリックすると、その女優の動画一覧にフィルタリング
...
└── data/                     # ランタイムデータ（git無視）
    ├── library.db
    ├── actress_data.json    # 945の女優プロフィールデータ
    └── images/
```
